...
export async function POST(request: NextRequest) {
  try {
    ...
    const model = genAI.getGenerativeModel({ 
      model: 'gemini-1.5-flash' // Use 1.5-flash for higher rate limits
    })
    
    const prompt = `You are an expert chess grandmaster AI playing as Black. 
                    Your objective is to WIN the game decisively.
                    Checkmate - it is position of Piece made by the player to trap oponents king in such a way that he can not move from it's position and the game ends
                    ...
                    🎯 Important: Do not just pick a move by priority order. Instead, evaluate ALL legal moves and choose the strongest one overall, balancing tactics and strategy.
                        ...
                        Don't Output Any Explanation Respond with ONLY one move from the legal moves list.
    ...
    if (error instanceof Error && error.message.includes('429')) {
      console.warn('Rate limit hit, returning null for fallback')
